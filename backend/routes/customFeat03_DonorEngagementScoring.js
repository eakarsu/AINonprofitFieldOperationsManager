...
// Time-series ML on donation patterns + volunteer participation to predict churn risk and recommend re-engagement campaigns
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINonprofitFieldOperationsManager / cf-donor-engagement-scoring',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_donor_engagement_scoring`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-donor-engagement-scoring feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Donor engagement scoring'.
Context: Time-series ML on donation patterns + volunteer participation to predict churn risk and recommend re-engagement campaigns
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-donor-engagement-scoring/run error:', err);
