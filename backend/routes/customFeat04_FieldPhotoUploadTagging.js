...
// Computer vision on incident/service photos; auto-extract location, condition severity, asset inventory
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINonprofitFieldOperationsManager / cf-field-photo-upload-tagging',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_field_photo_upload_tagging`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-field-photo-upload-tagging feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Field photo upload + tagging'.
Context: Computer vision on incident/service photos; auto-extract location, condition severity, asset inventory
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-field-photo-upload-tagging/run error:', err);
