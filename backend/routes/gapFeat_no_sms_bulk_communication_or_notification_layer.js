...
// Gap Feature: No SMS/bulk communication or notification layer
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINonprofitFieldOperationsManager / gap-no-sms-bulk-communication-or-notification-layer',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No SMS/bulk communication or notification layer'.\nProject: AINonprofitFieldOperationsManager.\nContext: No SMS/bulk communication or notification layer\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-sms-bulk-communication-or-notification-layer', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-sms-bulk-communication-or-notification-layer', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
