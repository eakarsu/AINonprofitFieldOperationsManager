...
// Gap Feature: No file/document storage for case attachments
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINonprofitFieldOperationsManager / gap-no-file-document-storage-for-case-attachments',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No file/document storage for case attachments'.\nProject: AINonprofitFieldOperationsManager.\nContext: No file/document storage for case attachments\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-file-document-storage-for-case-attachments', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-file-document-storage-for-case-attachments', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
