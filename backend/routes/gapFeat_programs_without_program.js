...
// Gap Feature: Programs without \'/program
// Programs without /program-risk-assessment (flag underperforming initiatives early)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINonprofitFieldOperationsManager / gap-programs-without-program',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Programs without \'/program'.\nProject: AINonprofitFieldOperationsManager.\nContext: Programs without /program-risk-assessment (flag underperforming initiatives early)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['programs-without-program', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'programs-without-program', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
