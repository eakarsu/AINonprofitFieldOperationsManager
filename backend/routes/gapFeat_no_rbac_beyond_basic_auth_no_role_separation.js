...
// Gap Feature: No RBAC beyond basic auth (no role separation
// No RBAC beyond basic auth (no role separation: coordinator vs. volunteer vs. donor)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINonprofitFieldOperationsManager / gap-no-rbac-beyond-basic-auth-no-role-separation',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No RBAC beyond basic auth (no role separation'.\nProject: AINonprofitFieldOperationsManager.\nContext: No RBAC beyond basic auth (no role separation: coordinator vs. volunteer vs. donor)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-rbac-beyond-basic-auth-no-role-separation', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-rbac-beyond-basic-auth-no-role-separation', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-rbac-beyond-basic-auth-no-role-separation']
    );
