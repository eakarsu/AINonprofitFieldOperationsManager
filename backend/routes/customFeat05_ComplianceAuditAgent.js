...
// Continuous background process that scans incident records, donations, and programs for grant compliance violations (funder restrictions, audit trail gaps)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINonprofitFieldOperationsManager / cf-compliance-audit-agent',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_compliance_audit_agent`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-compliance-audit-agent feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Compliance audit agent'.
Context: Continuous background process that scans incident records, donations, and programs for grant compliance violations (funder restrictions, audit trail gaps)
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-compliance-audit-agent/run error:', err);
