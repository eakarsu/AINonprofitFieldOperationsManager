...
// Gap Feature: No reporting export (PDF/CSV for board meetings and funders)
// No reporting export (PDF/CSV for board meetings and funders)
// Project: AINonprofitFieldOperationsManager
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINonprofitFieldOperationsManager / gap-no-reporting-export-pdf-csv-for-board-meetings-and',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No reporting export (PDF/CSV for board meetings and funders)'.\nProject: AINonprofitFieldOperationsManager.\nContext: No reporting export (PDF/CSV for board meetings and funders)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-reporting-export-pdf-csv-for-board-meetings-and', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-reporting-export-pdf-csv-for-board-meetings-and', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-reporting-export-pdf-csv-for-board-meetings-and']
    );
