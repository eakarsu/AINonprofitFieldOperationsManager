...
// Index nonprofit\'s case handling guidelines; let AI ingest and surface relevant precedents
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINonprofitFieldOperationsManager / cf-rag-over-organizational-playbooks',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_rag_over_organizational_playbooks`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-rag-over-organizational-playbooks feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'RAG over organizational playbooks'.
Context: Index nonprofit\'s case handling guidelines; let AI ingest and surface relevant precedents
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-rag-over-organizational-playbooks/run error:', err);
