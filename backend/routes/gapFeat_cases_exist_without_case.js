...
// Gap Feature: Cases exist without \'/case
// Cases exist without /case-resolution-predict (predict time-to-close, resource needs)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINonprofitFieldOperationsManager / gap-cases-exist-without-case',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Cases exist without \'/case'.\nProject: AINonprofitFieldOperationsManager.\nContext: Cases exist without /case-resolution-predict (predict time-to-close, resource needs)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['cases-exist-without-case', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'cases-exist-without-case', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
