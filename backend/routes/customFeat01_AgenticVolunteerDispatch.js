...
// Real-time multi-agent orchestration that auto-assigns volunteers, confirms availability via SMS, and reroutes if no-show detected
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINonprofitFieldOperationsManager / cf-agentic-volunteer-dispatch',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_volunteer_dispatch`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-agentic-volunteer-dispatch feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic volunteer dispatch'.
Context: Real-time multi-agent orchestration that auto-assigns volunteers, confirms availability via SMS, and reroutes if no-show detected
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-volunteer-dispatch/run error:', err);
