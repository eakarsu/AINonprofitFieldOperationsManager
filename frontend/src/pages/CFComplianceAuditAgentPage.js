...
// Custom Feature: Compliance audit agent
// Compliance audit agent: Continuous background process that scans incident records, donations, and programs for grant compliance violations (funder restrictions, audit trail gaps)
import React, { useState } from 'react';

export default function CFComplianceAuditAgentPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-compliance-audit-agent/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Compliance audit agent</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Compliance audit agent: Continuous background process that scans incident records, donations, and programs for grant compliance violations (funder restrictions, audit trail gaps)</p>
