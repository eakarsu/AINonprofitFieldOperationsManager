...
// Custom Feature: RAG over organizational playbooks
// RAG over organizational playbooks: Index nonprofit's case handling guidelines; let AI ingest and surface relevant precedents
import React, { useState } from 'react';

export default function CFRagOverOrganizationalPlaybooksPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-rag-over-organizational-playbooks/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>RAG over organizational playbooks</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>RAG over organizational playbooks: Index nonprofit's case handling guidelines; let AI ingest and surface relevant precedents</p>
