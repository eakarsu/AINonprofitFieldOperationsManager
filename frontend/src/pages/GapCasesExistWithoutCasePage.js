...
export default function GapCasesExistWithoutCasePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-cases-exist-without-case/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Cases exist without '/case</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Cases exist without /case-resolution-predict (predict time-to-close, resource needs)</p>
