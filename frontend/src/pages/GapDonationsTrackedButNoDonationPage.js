...
// Gap (AI): Donations tracked but no '/donation
// Donations tracked but no /donation-forecast (predict next gift propensity per donor)
import React, { useState } from 'react';

export default function GapDonationsTrackedButNoDonationPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-donations-tracked-but-no-donation/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Donations tracked but no '/donation</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Donations tracked but no /donation-forecast (predict next gift propensity per donor)</p>
