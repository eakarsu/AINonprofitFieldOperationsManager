...
// Gap (Non-AI): No RBAC beyond basic auth (no role separation
// No RBAC beyond basic auth (no role separation: coordinator vs. volunteer vs. donor)
import React, { useState } from 'react';

export default function GapNoRbacBeyondBasicAuthNoRoleSeparationPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-rbac-beyond-basic-auth-no-role-separation/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No RBAC beyond basic auth (no role separation</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No RBAC beyond basic auth (no role separation: coordinator vs. volunteer vs. donor)</p>
