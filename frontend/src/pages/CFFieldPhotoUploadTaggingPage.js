...
// Custom Feature: Field photo upload + tagging
// Field photo upload + tagging: Computer vision on incident/service photos; auto-extract location, condition severity, asset inventory
import React, { useState } from 'react';

export default function CFFieldPhotoUploadTaggingPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-field-photo-upload-tagging/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Field photo upload + tagging</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Field photo upload + tagging: Computer vision on incident/service photos; auto-extract location, condition severity, asset inventory</p>
