...
// Custom Feature: Donor engagement scoring
// Donor engagement scoring: Time-series ML on donation patterns + volunteer participation to predict churn risk and recommend re-engagement campaigns
import React, { useState } from 'react';

export default function CFDonorEngagementScoringPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-donor-engagement-scoring/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Donor engagement scoring</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Donor engagement scoring: Time-series ML on donation patterns + volunteer participation to predict churn risk and recommend re-engagement campaigns</p>
