...
// Gap (Non-AI): No multi
// No multi-language support (critical for nonprofits serving diverse populations)
import React, { useState } from 'react';

export default function GapNoMultiPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-multi/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No multi</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No multi-language support (critical for nonprofits serving diverse populations)</p>
